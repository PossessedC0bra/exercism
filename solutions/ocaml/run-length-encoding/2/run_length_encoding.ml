...
let decode s = 
  ...
  let _ = String.fold_left
    (fun count current_char -> 
      match count with
      | Some c when is_number current_char -> Some ((c * 10) + (to_integer current_char))
      | None when is_number current_char -> Some (to_integer current_char)
      | _ when is_letter current_char -> flush current_char count ; None
      | Some _ | None -> None
    )
    None
    s
